...
public class GuiFzInput extends Gui {

    protected int backgroundColor = BLACK_3;
    protected int fontColor = 0xFFFFFFFF;
    protected TTFFontRenderer fontRenderer;

    protected boolean textCenter = false;
    ...
    public GuiFzInput(int id, int x, int y, int width, int height) {
        this.id = id;
        this.x = x;
        this.y = y;
        this.width = width;
        this.height = height;
        this.fontRenderer = FzClient.getInstance().getTTFFontRenderers().get(24);
    }
    ...
    public void drawTextBox()
    {
        if (this.getVisible())
        {
            ...
            {
                RoundedShaderRenderer.getInstance().drawRoundRect(this.x, this.y, this.width, this.height, 4, backgroundColor);
            }
            ...
            if (!s.isEmpty())
            {
                ...
                if(this.textCenter) {
                    this.fontRenderer.drawCenteredString(s1, this.x + this.width/2 - this.fontRenderer.getWidth(s1)/2, i1, getFontColor());
                }
                else {
                    this.fontRenderer.drawString(s1, l, i1, getFontColor());
                }
            }
            ...
            if (!s.isEmpty() && flag && j < s.length())
            {
                if(this.textCenter) {
                    ...
                    this.fontRenderer.drawCenteredString(a, this.x + this.width/2 - this.fontRenderer.getWidth(a)/2, i1, getFontColor());
                }
                else {
                    this.fontRenderer.drawString(s.substring(j), l, i1, getFontColor());
                }
            }
            ...
            if (flag1)
            {
                if (flag2)
                {
                    Gui.drawRect(k1, i1 - 1, k1 + 1, i1 + 1 + this.fontRenderer.getHeight(), -3092272);
                }
                else
                {
                    if(this.textCenter) {
                        this.fontRenderer.drawCenteredString("_", this.x + this.width/2 - this.fontRenderer.getWidth("_")/2, i1, getFontColor());
                    }
                    else {
                        this.fontRenderer.drawString("_", l, i1, getFontColor());
                    }
                }
            }
            ...
            if (k != j)
            {
                ...
                this.drawSelectionBox(k1, i1 - 1, l1 - 1, i1 + 1 + this.fontRenderer.getHeight());
            }
        }
    }
    ...
    public GuiFzInput setBackgroundColor(int backgroundColor) {
        this.backgroundColor = backgroundColor;
        return this;
    }

    public GuiFzInput setFontColor(int textColor) {
    ...
    public GuiFzInput setTextCenter(boolean textCenter) {
