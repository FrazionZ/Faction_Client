...
public class GuiMarketItem extends GuiFrazionZInterface {
	
	private MarketItem item;
	private int totalSellItems = 1;
	private int totalBuyItems = 1;
	
	public GuiMarketItem(GuiScreen lastScreen, Minecraft mc, MarketItem item) {
	...
	protected void actionPerformed(GuiButton button, int mouseButton) throws IOException {
        switch (button.id)
        {
	    	case 1:
				...
				break;
        	case 2:
        		this.mc.player.connection.sendPacket(new CPacketShopTrade(EnumGui.MARKET_ITEM, this.item.getId(), this.totalBuyItems, 0));
        		break;
        	case 3:
        		this.mc.player.connection.sendPacket(new CPacketShopTrade(EnumGui.MARKET_ITEM, this.item.getId(), this.totalSellItems, 1));
        		break;
        	case 4:
        		this.mc.player.connection.sendPacket(new CPacketShopTrade(EnumGui.MARKET_ITEM, this.item.getId(), 3000, 1));
        		break;
        	case 5:
        		if(mouseButton == 0)
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        // Draw Prices
        String buyPrice = Double.toString(MathUtils.roundAvoid(this.totalBuyItems * this.item.getBuyPrice(), 2)) + " \u00A76$\u00A7r";
        String sellPrice = Double.toString(MathUtils.roundAvoid(this.totalSellItems * this.item.getSellPrice(), 2)) + " \u00A76$\u00A7r";
