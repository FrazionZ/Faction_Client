...
public class EntityDemonZ extends EntitySpellcasterIllager
{
    ...
    class AIAttackSpell extends EntitySpellcasterIllager.AIUseSpell
    {
        ...
        protected void castSpell()
        {
            ...
            if (EntityDemonZ.this.getDistanceSq(entitylivingbase) < 2.0D)
            {
                for (int i = 0; i < 5; ++i)
                {
                    float f1 = f + (float)i * (float)Math.PI * 0.4F;
                    this.spawnFangs(EntityDemonZ.this.posX + (double)MathHelper.cos(f1) * 1.5D, EntityDemonZ.this.posZ + (double)MathHelper.sin(f1) * 1.5D, d0, d1, f1, 0);
                }

                for (int k = 0; k < 8; ++k)
                {
                    float f2 = f + (float)k * (float)Math.PI * 2.0F / 8.0F + ((float)Math.PI * 2F / 5F);
                    this.spawnFangs(EntityDemonZ.this.posX + (double)MathHelper.cos(f2) * 2.5D, EntityDemonZ.this.posZ + (double)MathHelper.sin(f2) * 2.5D, d0, d1, f2, 3);
                }
            }
            else
            {
                for (int l = 0; l < 20; ++l)
                {
                    ...
                    this.spawnFangs(EntityDemonZ.this.posX + (double)MathHelper.cos(f) * d2, EntityDemonZ.this.posZ + (double)MathHelper.sin(f) * d2, d0, d1, f, j);
                }
            }
        }
        ...
        private void spawnFangs(double p_190876_1_, double p_190876_3_, double p_190876_5_, double p_190876_7_, float p_190876_9_, int p_190876_10_)
        {
            ...
            if (flag)
            {
                ...
                EntityDemonZ.this.world.spawnEntity(entityevokerfangs);
                EntityDemonZ.this.world.spawnEntity(entityevokerfangs2);
                EntityDemonZ.this.world.spawnEntity(entityevokerfangs3);
                EntityDemonZ.this.world.spawnEntity(entityevokerfangs4);
                EntityDemonZ.this.world.spawnEntity(entityevokerfangs5);
            }
        }
    ...
    class AICastingSpell extends EntitySpellcasterIllager.AICastingApell
    {
        ...
        public void updateTask()
        {
            ...
            else if (EntityDemonZ.this.getWololoTarget() != null)
            {
                EntityDemonZ.this.getLookHelper().setLookPositionWithEntity(EntityDemonZ.this.getWololoTarget(), (float)EntityDemonZ.this.getHorizontalFaceSpeed(), (float)EntityDemonZ.this.getVerticalFaceSpeed());
            }
        }
    }
    ...
    class AISummonSpell extends EntitySpellcasterIllager.AIUseSpell
    {
        ...
        public boolean shouldExecute()
        {
            ...
            {
                int i = EntityDemonZ.this.world.getEntitiesWithinAABB(EntityVindicator.class, EntityDemonZ.this.getEntityBoundingBox().grow(16.0D)).size();
        ...
        protected void castSpell()
        {
            for (int i = 0; i < 6; ++i)
            {
                ...
                EntityDemonZ.this.world.spawnEntity(entity);
            }
        }
