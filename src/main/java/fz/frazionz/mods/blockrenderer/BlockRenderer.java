...
public class BlockRenderer {
	...
	public void update(boolean exportForCss) {
		if (pendingBulkRender != null) {
			...
			if(exportForCss) {
				try {
					WebExporter.saveBlockRenderer();
				} catch (IOException e) {
					e.printStackTrace();
				}
			}
		}
		int code = mc.gameSettings.saveItemRenderers.getKeyCode();
		if (code > 256) {
			return;
		}
			...
			if (!down) {
				...
				if (GuiScreen.isCtrlKeyDown()) {
					...
					if (hovered != null && hovered.getHasStack()) {
						modid = Item.REGISTRY.getNameForObject(hovered.getStack().getItem()).getPath();
					}
	...
	private void bulkRender(String modidSpec, int size) {
		...
		File folder = new File("renders/" + sanitize(modidSpec)+"/");
		long lastUpdate = 0;
		...
		for (ItemStack is : toRender) {
			...
			if (Minecraft.getSystemTime()-lastUpdate > 33) {
				tearDownRenderState();
				renderLoading(I18n.format("Item Renderer", toRender.size(), joined),
						I18n.format("Rendu en cours...", rendered, toRender.size(), (toRender.size()-rendered)),
		...
		if (rendered >= toRender.size()) {
			renderLoading(I18n.format("Rendu Terminé", toRender.size(), Joiner.on(", ").join(modids)), "", null, 1);
		} else {
			renderLoading(I18n.format("Rendu Cancelled"),
					I18n.format("Rendu en cours...", rendered, toRender.size(), (toRender.size()-rendered)),
	...
	private String render(ItemStack is, File folder, boolean includeDateInFilename) {
		Minecraft mc = Minecraft.getMinecraft();
		String filename = sanitize(is.getDisplayName());
		GlStateManager.pushMatrix();
		GlStateManager.clearColor(0, 0, 0, 0);
		GlStateManager.clear(GL11.GL_COLOR_BUFFER_BIT | GL11.GL_DEPTH_BUFFER_BIT);
		mc.getRenderItem().renderItemWithoutEffectIntoGUI(is, 0, 0);
		GlStateManager.popMatrix();
		try {
			/*
			...
			String fileName = Item.getIdFromItem(is.getItem()) + "-" + is.getItemDamage() + (is.hasEffect() ? "_enchanted" : "" );
