...
public class ModKeystrokes extends ModDraggable {
	...
	public static enum KeystrokesMode {
		...
		WASD_SNEAK(Key.W, Key.A, Key.S, Key.D, Key.LMB, Key.RMB, new Key("\u00A7m-----", Minecraft.getMinecraft().gameSettings.keyBindJump, 1, 41, 58, 18, false)),
		WASD_SNEAK_MOUSE(Key.W, Key.A, Key.S, Key.LMB, Key.RMB, Key.D, new Key("\u00A7m-----", Minecraft.getMinecraft().gameSettings.keyBindJump, 1, 61, 58, 18, false));
	...
	private static class Key {
		...
		private static final Key W = new Key("^", Minecraft.getMinecraft().gameSettings.keyBindForward, 21, 1, 18, 18, false);
		private static final Key A = new Key("<", Minecraft.getMinecraft().gameSettings.keyBindLeft, 1, 21, 18, 18, false);
		private static final Key S = new Key("v", Minecraft.getMinecraft().gameSettings.keyBindBack, 21, 21, 18, 18, false);
		private static final Key D = new Key(">", Minecraft.getMinecraft().gameSettings.keyBindRight, 41, 21, 18, 18, false);
	...
	public void render(ScreenPosition pos) {
			...
			for(Key key : mode.getKeys()) {
				...
				font.drawString(
						...
						key.getName() == "^" ? pos.getAbsoluteY() + key.getY() + key.getHeight() / 2 - 1 : pos.getAbsoluteY() + key.getY() + key.getHeight() / 2 - 4, 
