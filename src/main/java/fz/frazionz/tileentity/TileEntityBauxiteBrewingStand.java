...
import fz.frazionz.inventory.ContainerBauxiteBrewingStand;
...
public class TileEntityBauxiteBrewingStand extends TileEntityLockable implements ITickable, ISidedInventory
{
    ...
    private static final int[] OUTPUT_SLOTS = new int[] {0, 1, 2, 3, 5};
    private NonNullList<ItemStack> brewingItemStacks = NonNullList.<ItemStack>withSize(6, ItemStack.EMPTY);
    ...
    {
        return this.hasCustomName() ? this.customName : "container.bauxite_brewing_stand";
    }
    ...
    public void update()
    {
        ItemStack itemstack = this.brewingItemStacks.get(5);
        ...
        ItemStack itemstack1 = this.brewingItemStacks.get(4);
        ...
        if (!this.world.isRemote)
        {
            ...
            if (!Arrays.equals(aboolean, this.filledSlots))
            {
                ...
                if (!(iblockstate.getBlock() instanceof BlockBauxiteBrewingStand))
                {
                    return;
                }

                for (int i = 0; i < BlockBauxiteBrewingStand.HAS_BOTTLE.length; ++i)
                {
                    iblockstate = iblockstate.withProperty(BlockBauxiteBrewingStand.HAS_BOTTLE[i], Boolean.valueOf(aboolean[i]));
                }
    ...
    private void brewPotions()
    {
        ...
        for (int i = 0; i < 4; ++i)
        {
            int size = this.brewingItemStacks.get(i).getStackSize();
            this.brewingItemStacks.set(i, PotionHelper.doReaction(itemstack, this.brewingItemStacks.get(i)));
            this.brewingItemStacks.get(i).setStackSize(size);
        }
        ...
        this.brewingItemStacks.set(4, itemstack);
        this.world.playEvent(1035, blockpos, 0);
    }

    public static void registerFixesBrewingStand(DataFixer fixer)
    {
        fixer.registerWalker(FixTypes.BLOCK_ENTITY, new ItemStackDataLists(TileEntityBauxiteBrewingStand.class, new String[] {"Items"}));
    }
    ...
    {
        return "minecraft:bauxite_brewing_stand";
    }

    public Container createContainer(InventoryPlayer playerInventory, EntityPlayer playerIn)
    {
        return new ContainerBauxiteBrewingStand(playerInventory, this);
    }
