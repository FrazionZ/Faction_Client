...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(float partialTicks)
    {
        ...
        //renderKeyStrokes(pos);


        
        if(this.hasIllegalTexture() || this.hasIllegalModel()) {
        	Config.dbg("--- Pack X-RAY Détecté ---");
        	Config.dbg("--- Pack X-RAY Détecté ---");
        	Config.dbg("--- Pack X-RAY Détecté ---");
    ...
    public static boolean hasIllegalBlockState()
    {
        ...
        if(sprite != null)
        {
            ...
            if(strs.length > 1)
            {
                String resource = strs[0] + ":blockstates/" + strs[1] + ".json";
    ...
    public static boolean hasIllegalModel()
    {
    	IBakedModel blockModel = Minecraft.getMinecraft().getBlockRendererDispatcher().getModelForState(Blocks.STONE.getDefaultState());
  
        if(blockModel != null)
        {
        	String ressource = "minecraft:models/block/" + 2 + ".json";
        	ResourceLocation ressourceLocation = new ResourceLocation(ressource);
        }
    ...
    public static boolean hasIllegalTexture()
    {
        ...
        sprite = Minecraft.getMinecraft().getBlockRendererDispatcher().getBlockModelShapes().getTexture(Blocks.GRASS.getDefaultState());
        ...
        sprite = Minecraft.getMinecraft().getBlockRendererDispatcher().getBlockModelShapes().getTexture(Blocks.DIRT.getDefaultState());
    ...
    private void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes)
    {
        ...
        for (Score score1 : collection)
        {
            ...
            drawRect(l1 - 2, k, l + 1, k + this.getFontRenderer().FONT_HEIGHT, 1342177280);
            
            // RENDER SCOREBOARD //
            // s2 = NUMBER SCOREBOARD //
            
            this.getFontRenderer().drawString(s1, l1, k, 553648127);
            this.getFontRenderer().drawString("", l - this.getFontRenderer().getStringWidth(""), k, 553648127);

            if (j == collection.size())
            {
                String s3 = objective.getDisplayName();
                drawRect(l1 - 2, k - this.getFontRenderer().FONT_HEIGHT - 1, l + 1, k - 1, 1610612736);
                drawRect(l1 - 2, k - 1, l + 1, k, 1342177280);
    ...
    private void renderPlayerStats(ScaledResolution scaledRes)
    {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer)
        {
            ...
            float f = entityplayer.getMaxHealth();
