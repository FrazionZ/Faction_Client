...
public class GuiKeyBindingList extends GuiListExtended
{
    ...
    {
        this(controls, mcIn, controls.width + 45, controls.height, 80, controls.height - 44, 20);
    }
    
    public GuiKeyBindingList(GuiControls controls, Minecraft mcIn, int width, int height, int top, int bottom, int slotHeight)
    {
    ...
    public GuiKeyBindingList(GuiControls controls, Minecraft mcIn, int left, int right, int top, int bottom, int slotHeight, boolean second)
    {
        super(mcIn, left, right, top, bottom, slotHeight, second);
        this.controlsScreen = controls;
        this.mc = mcIn;
        KeyBinding[] akeybinding = (KeyBinding[])ArrayUtils.clone(mcIn.gameSettings.keyBindings);
        this.listEntries = new GuiListExtended.IGuiListEntry[akeybinding.length + KeyBinding.getKeybinds().size()];
    ...
    public class CategoryEntry implements GuiListExtended.IGuiListEntry
    {
        ...
        {
            GuiKeyBindingList.this.mc.fontRendererObj.drawString(this.labelText, GuiKeyBindingList.this.left + (GuiKeyBindingList.this.right - GuiKeyBindingList.this.left)/2 - this.labelWidth / 2, p_192634_3_ + p_192634_5_ - GuiKeyBindingList.this.mc.fontRendererObj.FONT_HEIGHT - 1, 16777215);
        }
