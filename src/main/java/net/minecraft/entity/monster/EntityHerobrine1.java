...
public class EntityHerobrine1 extends EntitySpellcasterIllager
{
    ...
    public static void registerFixesHerobrine(DataFixer fixer)
    {
    ...
    class AIAttackSpell extends EntitySpellcasterIllager.AIUseSpell
    {
        ...
        protected void func_190868_j()
        {
            ...
            if (EntityHerobrine1.this.getDistanceSqToEntity(entitylivingbase) < 2.0D)
            {
                for (int i = 0; i < 5; ++i)
                {
                    float f1 = f + (float)i * (float)Math.PI * 0.4F;
                    this.func_190876_a(EntityHerobrine1.this.posX + (double)MathHelper.cos(f1) * 1.5D, EntityHerobrine1.this.posZ + (double)MathHelper.sin(f1) * 1.5D, d0, d1, f1, 0);
                }

                for (int k = 0; k < 8; ++k)
                {
                    float f2 = f + (float)k * (float)Math.PI * 2.0F / 8.0F + ((float)Math.PI * 2F / 5F);
                    this.func_190876_a(EntityHerobrine1.this.posX + (double)MathHelper.cos(f2) * 2.5D, EntityHerobrine1.this.posZ + (double)MathHelper.sin(f2) * 2.5D, d0, d1, f2, 3);
                }
            }
            else
            {
                for (int l = 0; l < 20; ++l)
                {
                    ...
                    this.func_190876_a(EntityHerobrine1.this.posX + (double)MathHelper.cos(f) * d2, EntityHerobrine1.this.posZ + (double)MathHelper.sin(f) * d2, d0, d1, f, j);
                }
            }
        }
        ...
        private void func_190876_a(double p_190876_1_, double p_190876_3_, double p_190876_5_, double p_190876_7_, float p_190876_9_, int p_190876_10_)
        {
            ...
            if (flag)
            {
                ...
                EntityHerobrine1.this.world.spawnEntityInWorld(entityevokerfangs);
                EntityHerobrine1.this.world.spawnEntityInWorld(entityevokerfangs2);
                EntityHerobrine1.this.world.spawnEntityInWorld(entityevokerfangs3);
                EntityHerobrine1.this.world.spawnEntityInWorld(entityevokerfangs4);
                EntityHerobrine1.this.world.spawnEntityInWorld(entityevokerfangs5);
            }
        }
    ...
    class AICastingSpell extends EntitySpellcasterIllager.AICastingApell
    {
        ...
        public void updateTask()
        {
            ...
            else if (EntityHerobrine1.this.func_190751_dj() != null)
            {
                EntityHerobrine1.this.getLookHelper().setLookPositionWithEntity(EntityHerobrine1.this.func_190751_dj(), (float)EntityHerobrine1.this.getHorizontalFaceSpeed(), (float)EntityHerobrine1.this.getVerticalFaceSpeed());
            }
        }
    }
    ...
    class AISummonSpell extends EntitySpellcasterIllager.AIUseSpell
    {
        ...
        public boolean shouldExecute()
        {
            ...
            {
                int i = EntityHerobrine1.this.world.getEntitiesWithinAABB(EntityVindicator.class, EntityHerobrine1.this.getEntityBoundingBox().expandXyz(16.0D)).size();
        ...
        protected void func_190868_j()
        {
            for (int i = 0; i < 6; ++i)
            {
                ...
                EntityHerobrine1.this.world.spawnEntityInWorld(entity);
            }
        }
