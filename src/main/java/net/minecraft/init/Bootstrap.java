...
public class Bootstrap
{
    ...
    static void registerDispenserBehaviors()
    {
        ...
        BlockDispenser.DISPENSE_BEHAVIOR_REGISTRY.putObject(Items.BIG_XP, new BehaviorProjectileDispense()
        {
            protected IProjectile getProjectileEntity(World worldIn, IPosition position, ItemStack stackIn)
            {
                return new EntityBigXp(worldIn, position.getX(), position.getY(), position.getZ());
            }
            protected float getProjectileInaccuracy()
            {
                return super.getProjectileInaccuracy() * 0.5F;
                    ...
                    if (!world.isRemote)
                    {
                        ...
                        if (tileentity instanceof TileEntitySkull)
                        {
                            if (stack.getMetadata() == 3)
                            {
                                ...
                                ((TileEntitySkull)tileentity).setPlayerProfile(gameprofile, 3);
                            }
                            else
                            {
        ...
        BlockDispenser.DISPENSE_BEHAVIOR_REGISTRY.putObject(Item.getItemFromBlock(Blocks.Z_TNT), new BehaviorDefaultDispenseItem()
        {
            protected ItemStack dispenseStack(IBlockSource source, ItemStack stack)
            {
                World world = source.getWorld();
