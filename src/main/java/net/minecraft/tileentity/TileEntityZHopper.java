...
import net.minecraft.inventory.ContainerZHopper;
...
public class TileEntityZHopper extends TileEntityLockableLoot implements IHopper, ITickable
{
    private NonNullList<ItemStack> inventory = NonNullList.<ItemStack>func_191197_a(18, ItemStack.field_190927_a);
    private int transferCooldown = -1;
    private long field_190578_g;

    public static void registerFixesHopper(DataFixer fixer)
    {
        fixer.registerWalker(FixTypes.BLOCK_ENTITY, new ItemStackDataLists(TileEntityZHopper.class, new String[] {"Items"}));
    }
    ...
    {
        return this.hasCustomName() ? this.field_190577_o : "container.z_hopper";
    }
    ...
    private boolean updateHopper()
    {
        if (this.world != null && !this.world.isRemote)
        {
            if (!this.isOnTransferCooldown() && BlockZHopper.isEnabled(this.getBlockMetadata()))
            {
                boolean flag = false;
    ...
    private boolean transferItemsOut()
    {
        ...
        {
            EnumFacing enumfacing = BlockZHopper.getFacing(this.getBlockMetadata()).getOpposite();
    ...
    public static boolean captureDroppedItems(IHopper hopper)
    {
        ...
        {
            for (EntityItem entityitem : func_181556_a(hopper.getWorld(), hopper.getXPos(), hopper.getYPos(), hopper.getZPos()))
            {
    ...
    private static ItemStack insertStack(IInventory inventoryIn, IInventory stack, ItemStack index, int side, EnumFacing p_174916_4_)
    {
        ...
        if (canInsertItemInSlot(stack, index, side, p_174916_4_))
        {
            ...
            if (flag)
            {
                if (flag1 && stack instanceof TileEntityZHopper)
                {
                    TileEntityZHopper tileentityhopper1 = (TileEntityZHopper)stack;

                    if (!tileentityhopper1.mayTransfer())
                    {
                        int k = 0;

                        if (inventoryIn != null && inventoryIn instanceof TileEntityZHopper)
                        {
                            TileEntityZHopper tileentityhopper = (TileEntityZHopper)inventoryIn;
                        ...
                        tileentityhopper1.setTransferCooldown(4 - k);
                    }
                }
    ...
    {
        EnumFacing enumfacing = BlockZHopper.getFacing(this.getBlockMetadata());
    ...
    public static List<EntityItem> func_181556_a(World p_181556_0_, double p_181556_1_, double p_181556_3_, double p_181556_5_) // WEST - BAS - NORD || EST - HAUTEUR - SUD //
    {
        return p_181556_0_.<EntityItem>getEntitiesWithinAABB(EntityItem.class, new AxisAlignedBB(p_181556_1_ - 4.5D, p_181556_3_ - 1.0D, p_181556_5_ - 4.5D, p_181556_1_ + 4.5D, p_181556_3_ + 4.5D, p_181556_5_ + 4.5D), EntitySelectors.IS_ALIVE);
    }
    /**
    ...
    public Container createContainer(InventoryPlayer playerInventory, EntityPlayer playerIn)
    {
        ...
        return new ContainerZHopper(playerInventory, this, playerIn);
    }
